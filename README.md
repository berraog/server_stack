...
| games | ES-DE, RetroArch | the TV — HDMI out, no port at all |
...
/mnt/active/           1 TB USB — everything torrents touch
  ...
  games/roms/<system>/ the tree ES-DE reads; hardlinked out of other/ (§16)
...
~/stack/                          THIS repo — how the machine fits together
  ...
  games/compose.yaml              es-de, retroarch — the TV (§16)
  ...
  bin/roms-import                 hardlink downloaded ROMs into ES-DE's tree (§16)
  bin/games-link-emulators        let ES-DE find RetroArch across containers (§16)
...
| — | ES-DE, RetroArch | no port by design: the output is HDMI (§16) |
...
## 16. The games stack — ES-DE on the TV

A retro console on the same box, on the HDMI output, driven by an Xbox pad over
Bluetooth. [`games/compose.yaml`](games/compose.yaml) is short and commented;
...
### 16.2 Getting ROMs in — `bin/roms-import`
...
`downloads/` and `library/` in §1. One set of blocks, two names: the download
...
### 16.3 Bluetooth — pair on the host, not in the container
...
### 16.4 Connecting ES-DE to the emulators — `bin/games-link-emulators`
...
### 16.6 When it does not work

| Symptom | Cause |
| --- | --- |
| TV stays on the console login | Xorg could not take the VT. `sudo systemctl mask getty@tty1` and restart `es-de`. |
| ES-DE starts, no games | The ROM tree is not laid out per system. `bin/roms-import` (§16.2), then restart `es-de`. |
| Games listed, every launch fails | The shim is missing. `bin/games-link-emulators` (§16.4). |
...
| Pad works, then not after a reboot | `trust <MAC>` was never run (§16.3). |
...
port and is not exposed through the tunnel (§14); it is reachable only from the
